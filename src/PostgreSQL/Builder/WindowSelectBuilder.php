 ...
 * {@see partitionBy()} and {@see orderBy()} define the window named by the
 * preceding {@see SelectBuilder::window()} call.
 */
class WindowSelectBuilder extends SelectBuilder
{
    ...
    public function as(string $existingWindowName = ''): self
    {
        ...
        return $this->deriveWindow(self::class, new WindowDefinition($existingWindowName, $def->partitionBy, $def->orderBys));
    }
