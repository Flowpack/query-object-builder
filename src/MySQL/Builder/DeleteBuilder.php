 ...
 * and `LIMIT`. Joining further tables via {@see join()} turns it into a
 * multi-table delete (`DELETE tbl.* FROM tbl JOIN ...`), where those clauses are
 * not allowed; the joined tables only restrict which rows of the target are
 * deleted.
 *
...
class DeleteBuilder implements InnerSqlWriter
{
    ...
    private function addJoin(JoinType $joinType, FromExp $from): JoinDeleteBuilder
    {
        return $this->derive(JoinDeleteBuilder::class, joins: [...$this->joins, new FromItem(new Join($joinType, false, $from))]);
    }
    ...
    {
        return $this->derive(OrderByDeleteBuilder::class, orderBys: [...$this->orderBys, new OrderByClause($exp)]);
    }
    ...
    {
        // ORDER BY / LIMIT bound which rows a single-table delete touches; they are
        // not part of the multi-table grammar.
        if ($this->joins !== [] && ($this->orderBys !== [] || $this->limit !== null)) {
            $sb->addError(new QueryBuilderException('delete: ORDER BY / LIMIT not allowed in a multi-table delete'));
